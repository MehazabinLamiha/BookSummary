...
Deliberately styled differently from the Custome"""
FAISS Book Search Dashboard — Book Summaries
----------------------------------------------
A professional Streamlit dashboard for the "Book Summaries" FAISS lab.
...
with tab_data:
    ...
    with col_chart:
        ...
        fig3.tight_layout()
        st.pyplot(fig3)r Reviews dashboard
(green/yellow theme, dropdown query"""
FAISS Book Search Dashboard — Book Summaries
----------------------------------------------
A professional Streamlit dashboard for the "Book Summaries" FAISS lab.
Deliberately styled differently from the Customer Reviews dashboard
...
# import time. On a 20-vector demo dataset there is zero benefit to multi-
# threading any of these indexes, but two separate thread pools can otherwise
# add per-call overhead that swamps the actual (sub-microsecond) computation:
#   - faiss.omp_set_num_threads() (set further below) only covers FAISS's own
#     OpenMP parallel regions (used by IndexPQ / IndexIVFPQ's search).
#   - OpenBLAS/MKL/etc. run the matrix multiply inside PQ's distance-table
#     computation through their OWN independent thread pool, controlled only
#     by these env vars — faiss.omp_set_num_threads() does not touch it.
# Without both, PQ-based indexes can still show inflated, inconsistent
# latency (and IVF+PQ won't reliably beat plain PQ) depending on the host's
# core count, even with the FAISS-level fix in place.
...
faiss.omp_set_num_threads(1)  # also cap FAISS's own OpenMP regions, belt-and-braces
import matplotlib.pyplot as plt
import numpy as np
import pandas as pd
import streamlit as st
...
with tab_data:
    ...
    with col_chart:
        ...
        ax3.set_title("Genre Distribution", fontsize=13, fontweight="bold", pad=14)
        fig3.tight_layout()
        st.pyplot(fig3) picker, KPI cards, extra analytics)
while following the same underlying database-driven pattern.
